...
def extract_bow_histogram(image_path, sift, kmeans):
    image = cv2.imread(image_path)
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
...
for path in tqdm(image_paths):
    hist = extract_bow_histogram(path, sift, kmeans)
...
joblib.dump(kmeans, 'saved_model/sift_kmeans.pkl')
joblib.dump(svm, 'saved_model/sift_svm.pkl')
print("Models saved!")
